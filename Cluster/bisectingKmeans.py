...
class BisectingKmeans:
    ...
    def fit(self, data):
        ...
        for i in range(1, self.max_k):
            ...
            kmeans = KMeans(n_clusters=2, verbose=False, init=self.init, metric=self.metric)
            kmeans.fit(data[:, 1:embedding_dim])

            c1, c2 = [], []
            for j in range(len(data)):
                if kmeans.clusters[j] == 0:
                    c1.append(data[j])
                else:
                    c2.append(data[j])
            ...
            self.G.add_node("{:.3f}".format(sse_c1), kmeans=kmeans, data=c1[:, 0])
            self.G.add_node("{:.3f}".format(sse_c2), kmeans=kmeans, data=c2[:, 0])
            if i == 1: #First node (root)
                self.G.add_node(new_max_sse, kmeans=kmeans, data=data[:, 0])
