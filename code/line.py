...
class LINEModel:
    def __init__(self, args):
        self.u_i = tf.placeholder(name='u_i', dtype=tf.int32, shape=[args.batch_size * (args.window + 1)])
        self.u_j = tf.placeholder(name='u_j', dtype=tf.int32, shape=[args.batch_size * (args.window + 1)])
        self.label = tf.placeholder(name='label', dtype=tf.float32, shape=[args.batch_size * (args.window + 1)])
...
def train(args):
    data_loader = DBLPDataLoader(graph_file=args.input)
    ...
    with tf.Session() as sess:
        ...
        for b in range(args.num_batches):
            t1 = time.time()
            u_i, u_j, label = data_loader.fetch_batch(batch_size=args.batch_size, K=args.window)
            ...
            if b % 1000 == 0 or b == (args.num_batches - 1):
                ...
                pickle.dump(data_loader.embedding_mapping(normalized_embedding),
                            open('embeddings/embedding_%s.pkl' % suffix, 'wb'))

    load_dataset(args)
    
def load_dataset(args):
    
    file = 'embedding_%s.pkl' %args.proximity
    infile = open('embeddings/%s' %file, 'rb')
    embedding = pickle.load(infile)
    infile.close()
    dirr = os.listdir('embeddings')
    if file in dirr:
        os.remove('embeddings/%s' %file)
    
    external_list = []
    for node in embedding.items():
        internal_list = []
        internal_list.append(node[0])
        for i in range(len(node[1])):
            internal_list.append(node[1][i])
        external_list.append(internal_list)

    aux = pd.DataFrame(external_list)
    aux.to_csv('embeddings/%s' %args.output, sep=',', header=False, index=False)
