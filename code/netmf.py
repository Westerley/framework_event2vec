...
def load_adjacency_matrix(file):
    data = nx.read_gpickle(file) 
    data = nx.to_numpy_matrix(data, nodelist=data.nodes())
    data = sparse.csr_matrix(data) 
    logger.info("loading pkl file %s", file)
    return data
...
def netmf_large(args):
    ...
    deepwalk_matrix = approximate_deepwalk_matrix(evals, D_rt_invU,
            window=args.window,
            vol=vol, b=args.negative_samples)

    # factorize deepwalk matrix with SVD
    deepwalk_embedding = svd_deepwalk_matrix(deepwalk_matrix, dim=args.embedding_dim)

    logger.info("Save embedding to %s", args.output)
    np.save('embeddings/%s' %args.output, deepwalk_embedding, allow_pickle=False)
    load_dataset(args)
...
def netmf_small(args):
    ...
    deepwalk_matrix = direct_compute_deepwalk_matrix(A,
            window=args.window, b=args.negative_samples)

    # factorize deepwalk matrix with SVD
    deepwalk_embedding = svd_deepwalk_matrix(deepwalk_matrix, dim=args.embedding_dim)
    logger.info("Save embedding to %s", args.output)
    np.save('embeddings/%s' %args.output[:-4], deepwalk_embedding, allow_pickle=False)
    load_dataset(args)

def load_dataset(args):
    data = nx.read_gpickle(args.input) 
    nodes = []
    for n in data.nodes():
        nodes.append(n)
    ...
    embedding = np.load('embeddings/%s.npy' %args.output[:-4]) 
    embedding = np.concatenate((nodes, embedding), axis=1)
    np.savetxt('embeddings/%s'%args.output, embedding, delimiter=',', fmt='%s')
    file = '%s.npy' %args.output[:-4]
