...
class Dataset(object):
    def __init__(self, nodes, edges, window_size):
        index2token,token2index,word_and_counts,index2frequency,node_context_pairs= self.parse_random_walk_txt(nodes, edges, window_size)
        self.window_size = window_size
        self.nodeid2index = token2index
        self.index2nodeid = index2token
        self.index2frequency = index2frequency
        index2type,type2indices = self.parse_node_type_mapping_txt(nodes)
    ...
    def parse_node_type_mapping_txt(self, nodes):
        index2type={}

        index2type = nodes['type'].to_dict()
    ...
    def parse_random_walk_txt(self, nodes, edges, window_size):

        word_and_counts = nodes['node'].value_counts()
        word_and_counts = word_and_counts.to_dict()

        print("The number of unique words:%d"%len(word_and_counts))
        index2token = nodes['node'].to_dict()
        ...
        for i, col in edges.iterrows():
            node_context_pairs.append((token2index[col['node1'].strip()], token2index[col['node2'].strip()]))
...
def train(center_node_placeholder,context_node_placeholder,negative_samples_placeholder,loss,dataset,optimizer,NUM_EPOCHS,BATCH_SIZE,NUM_SAMPLED,care_type,LOG_DIRECTORY,LOG_INTERVAL,MAX_KEEP_MODEL,OUTPUT):
    '''
...
def load_dataset(OUTPUT):
    index2nodeid = json.load(open(os.getcwd() + '/code/log/index2nodeid.json'))
    index2nodeid = {int(k):v for k,v in index2nodeid.items()}
    nodeid2index = {v:int(k) for k,v in index2nodeid.items()}
    node_embeddings = np.load(os.getcwd() + '/code/log/node_embeddings.npz')['arr_0']

    metapath2vec = pd.DataFrame(node_embeddings)
    aux = pd.DataFrame()
    aux['node'] = [k for k,v in nodeid2index.items()]
    for i in range(metapath2vec.shape[1]):
        aux[i] = metapath2vec[i]
    aux.to_csv('embeddings/%s'%OUTPUT, index=False, header=False)
